...
type GroupedPool = {
   ...
   */
  export async function fetchQuickswapGroupedSubgraph(poolIds: string[]) {
    ...
    const res = await fetch(`/api/quickswap-grouped?${params.toString()}`);
    if (!res.ok) {
      throw new Error(`Error fetching quickswap grouped data. poolIds: ${ids.join(",")}`);
    }
